...
class DetectionController extends Controller
{
    public function index(Request $request)
    {
        ...
        $operators = []; // Default kosong untuk User biasa

        // Jika User BUKAN Admin, hanya tampilkan miliknya sendiri
        if (!$user->isAdmin()) {
            $query->where('user_id', $user->id);
        } 
        // Jika ADMIN, jalankan fitur Filter
        else {
            // Ambil daftar operator yang memiliki riwayat deteksi beserta jumlahnya
            $operators = \App\Models\User::withCount('detections')->having('detections_count', '>', 0)->get();

            // Jika Admin memilih filter di Dropdown
        ...
        // Tetap bawa query string filter saat pindah halaman paginasi
    ...
    {
        // Keamanan: Hanya pemilik atau Admin yang boleh melihat
    ...
    {
        // Keamanan: Hanya pemilik atau Admin yang boleh menghapus
        if (Auth::id() !== $detection->user_id && !Auth::user()->isAdmin()) {
            abort(403, 'Akses ditolak. Anda hanya dapat menghapus riwayat milik sendiri.');
        }

        // Hapus file gambar dari Storage server
    ...
    public function clear()
    {
        ...
        // Jika bukan admin, hanya hapus miliknya sendiri
        if (!$user->isAdmin()) {
            $query->where('user_id', $user->id);
        }
